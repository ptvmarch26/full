...
function main() {
  const [, , voterArg, candidateArg, selectionArg, trusteeArg, thresholdArg] =
    process.argv;
  ...
  const numTrustees = Number.parseInt(trusteeArg, 10);
  const threshold = Number.parseInt(thresholdArg, 10);

  if (!isPositiveInt(numVoters)) {
    fail(
      "The number of voters must be a positive integer. Example: node scripts/prepare.js 1000 5 1 3 2",
    );
  }

  if (!isPositiveInt(numCandidates)) {
    fail(
      "The number of candidates must be a positive integer. Example: node scripts/prepare.js 1000 5 1 3 2",
    );
  }

  if (!isPositiveInt(numSelections)) {
    fail(
      "The number of selections must be a positive integer. Example: node scripts/prepare.js 1000 5 1 3 2",
    );
  }

  if (!isPositiveInt(numTrustees)) {
    fail(
      "The number of trustees must be a positive integer. Example: node scripts/prepare.js 1000 5 1 3 2",
    );
  }

  if (!isPositiveInt(threshold)) {
    fail(
      "The threshold must be a positive integer. Example: node scripts/prepare.js 1000 5 1 3 2",
    );
  }
  ...
  if (threshold > numTrustees) {
    fail("The threshold cannot be greater than the number of trustees.");
  }
  ...
  const files = {
    ...
    partial: findFirstExisting(projectRoot, [
      "test/partial.js",
      "scripts/partial.js",
      "partial.js",
    ]),
    tally: findFirstExisting(projectRoot, [
      "test/tally.js",
      "scripts/tally.js",
      "tally.js",
    ]),
    aggregate: findFirstExisting(projectRoot, [
      "test/aggregate.js",
  ...
  updateFile(files.tallyValidity, (content) => {
    ...
    next = replaceOrFail(
      ...
      `component main = BatchTallyValidity(${threshold}, ${numCandidates});`,
      files.tallyValidity,
    );
  ...
  updateFile(files.register, (content) => {
    ...
    next = replaceOrFail(
      ...
      `${files.register} (NUM_CANDIDATES)`,
    );

    next = replaceOrFail(
      next,
      /const\s+TOTAL_TRUSTEES\s*=\s*\d+\s*;/,
      `const TOTAL_TRUSTEES = ${numTrustees};`,
      `${files.register} (TOTAL_TRUSTEES)`,
    );

    next = replaceOrFail(
      next,
      /const\s+THRESHOLD\s*=\s*\d+\s*;/,
      `const THRESHOLD = ${threshold};`,
      `${files.register} (THRESHOLD)`,
    );
  ...
  updateFile(files.vote, (content) => {
    ...
    next = replaceOrFail(
      ...
      /const\s+THRESHOLD\s*=\s*\d+\s*;/,
      `const THRESHOLD = ${threshold};`,
      `${files.vote} (THRESHOLD)`,
    );

    return next;
  });

  updateFile(files.partial, (content) => {
    let next = content;
    ...
    next = replaceOrFail(
      ...
      /const\s+THRESHOLD\s*=\s*\d+\s*;/,
      `const THRESHOLD = ${threshold};`,
      `${files.partial} (THRESHOLD)`,
    );

    return next;
  });

  updateFile(files.tally, (content) => {
    let next = content;
    ...
    next = replaceOrFail(
      ...
      /const\s+THRESHOLD\s*=\s*\d+\s*;/,
      `const THRESHOLD = ${threshold};`,
      `${files.tally} (THRESHOLD)`,
    );

    return next;
  });

    updateFile(files.aggregate, (content) => {
    let next = content;
    ...
    next = replaceOrFail(
      ...
      /const\s+TOTAL_TRUSTEES\s*=\s*\d+\s*;/,
      `const TOTAL_TRUSTEES = ${numTrustees};`,
      `${files.aggregate} (TOTAL_TRUSTEES)`,
    );
  ...
  console.log(`- numTrustees   : ${numTrustees}`);
  ...
  console.log(`- ${files.partial}`);
  console.log(`- ${files.tally}`);
  console.log(`- ${files.aggregate}`);
}

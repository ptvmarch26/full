...
function computeLagrangeCoefficients(ids, modulus) {
  return ids.map((idJ, j) => {
    let num = 1n;
    let den = 1n;

    for (let m = 0; m < ids.length; m++) {
      if (m === j) continue;
      num = (num * -ids[m]) % modulus;
      den = (den * (idJ - ids[m])) % modulus;
    }

    return (((num * modInverse(den, modulus)) % modulus) + modulus) % modulus;
  });
}
...
async function main() {
  ...
  if (partialEvents.length < THRESHOLD) {
    throw new Error(
      `Need at least ${THRESHOLD} PartialDecryptionSubmitted events.`,
    );
  }
  ...
  for (const event of partialEvents) {
    ...
    if (seen.has(trustee)) continue;
    seen.add(trustee);

    const onChainId = await reader.trusteeID(event.args.trustee);
    const id = BigInt(onChainId.toString());
    ...
    trusteeDecryptions.push({
      trustee,
      id,
      points,
    });

    if (trusteeDecryptions.length >= THRESHOLD) break;
  }

  if (trusteeDecryptions.length < THRESHOLD) {
    throw new Error("Not enough unique trustee decryptions.");
  }

  const ids = trusteeDecryptions.map((x) => x.id);
  const lambdas = computeLagrangeCoefficients(ids, n);

  const nCandidates = C2TotalX.length;

  for (const item of trusteeDecryptions) {
    if (item.points.length < nCandidates) {
  ...
  const inputObject = {
    ...
    D_x: [],
    D_y: [],
    lambda: lambdas.map((x) => x.toString()),
    Mx: [],
    My: [],
  };
  ...
  for (let i = 0; i < nCandidates; i++) {
    ...
    for (let j = 0; j < trusteeDecryptions.length; j++) {
      const Dj = trusteeDecryptions[j].points[i].map((value) =>
        F.e(BigInt(value)),
      );

      const DjScaled = babyjub.mulPointEscalar(Dj, lambdas[j]);
      sumD = babyjub.addPoint(sumD, DjScaled);

      dxRow.push(trusteeDecryptions[j].points[i][0]);
      dyRow.push(trusteeDecryptions[j].points[i][1]);
    }
    ...
    inputObject.D_x.push(dxRow);
    inputObject.D_y.push(dyRow);
  ...
  console.log("Results revealed on-chain.");
  process.exit(0);
}

...
async function main() {
  ...
  for (let i = 1; i <= TOTAL_TRUSTEES; i++) {
    if (published >= THRESHOLD) break;

    const keyFile = path.join(DKG_PATH, `Trustee_${i}.json`);
    if (!fs.existsSync(keyFile)) {
      continue;
    }

    const { votingContract } = await getContract(i);
    ...
    console.log(`Generating proof for Trustee_${i}`);
  ...
  console.log(`Partial decryption total time: ${partialDecryptionTime} ms`);
  process.exit(0);
}
